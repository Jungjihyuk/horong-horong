...
def _render_detail_event(event: TimelineEvent) -> list[str]:
    head = (
        ...
        f"`{event.rank}순위 · 중요도 {event.importance}` — {event.title}"
    )
    lines = [head]
    lines += [f"  - {bullet}" for bullet in event.bullets]
    if event.tags:
...
def render_timeline_markdown(state: TimelineState) -> str:
    ...
    turning = [month.month_key for month in state.months if month.is_turning_point]

    header = [
        f"# 📊 {state.category_label} — 월별 핵심 사건 타임라인",
        "",
        f"{state.date_from} ~ {state.date_to} · {total_events}개 시점 · "
        f"{len(state.months)}개월 · 월별 축 최대 {MAX_AXIS_EVENTS}개 · "
        f"전환점 {len(turning)}곳",
    ]
    ...
    for month in state.months:
        body += [
            ...
            "",
            f"## 📅 {month.month_key}"
            + (" 🔀 전환점" if month.is_turning_point else ""),
            "",
            f"`축 {len(month.axis_events)} · 세부 {len(month.detail_events)}`"
