...
def plan_months(
    ...
    *,
    ...
    for month_key, month_events in sorted(group_by_month(events).items()):
        digest = month_input_hash(
            month_events, prompt_version=prompt_version, axis_limit=axis_limit
        )
        ...
        elif previous is not None and previous.axis_limit != axis_limit:
            reason, needs = "axis-limit", True
