...
final class GoalSuggestionEvalTests: XCTestCase {
    func testGenerateGoldenSetResults() async throws {
        ...
        for goldenCase in weeklyCases { await run(goldenCase, provider: provider, model: model, runner: runner, runID: runID) }
        for goldenCase in monthlyCases { await run(goldenCase, provider: provider, model: model, runner: runner, runID: runID) }
        runner.flush()
        print("[eval] 주간 \(weeklyCases.count)개, 월간 \(monthlyCases.count)개 → \(outputFile.path)")
    }

    private func selectedProvider() -> Constants.AchievementSuggestionProviderKind {
    ...
    private func run(_ goldenCase: GoldenSet.Case, provider: Constants.AchievementSuggestionProviderKind, model: String, runner: RunLogger, runID: String) async {
        ...
        let result = await AchievementFoundationGoalSuggestionProvider.suggestions(
            ...
            recommendationContext: goldenCase.context?.taskContext ?? .empty, provider: provider
        )
        record(caseName: goldenCase.caseName, expectedGroups: goldenCase.expectedMemoGroups(of: .weekly), expectedOutcome: goldenCase.expectedOutcome, expectedInputIDs: goldenCase.identifiers.shortIDByUUID, result: result, task: "weekly_goal", provider: provider.rawValue, model: model, recipe: goldenCase.context == nil ? "promptOnly" : "promptWithContext", startedAt: startedAt, runID: runID, traps: goldenCase.traps ?? [], runner: runner)
    }

    private func run(_ goldenCase: GoldenSet.MonthlyCase, provider: Constants.AchievementSuggestionProviderKind, model: String, runner: RunLogger, runID: String) async {
        ...
        let result = await AchievementFoundationGoalSuggestionProvider.monthlySuggestions(
            ...
            runID: runID, recommendationContext: goldenCase.context?.taskContext ?? .empty, provider: provider
        )
        record(caseName: goldenCase.caseName, expectedGroups: goldenCase.expectedGoalGroups(), expectedOutcome: goldenCase.expectedOutcome, expectedInputIDs: goldenCase.identifiers.shortIDByUUID, result: result, task: "monthly_goal", provider: provider.rawValue, model: model, recipe: goldenCase.context == nil ? "promptOnly" : "promptWithContext", startedAt: startedAt, runID: runID, traps: goldenCase.traps ?? [], runner: runner)
    }
