...
현재 소스는 `Features/`, `Models/`, `Services/` 폴더가 완전히 소멸되었으며, `Utilities/`의 잔여 공통 모듈을 제외하고 Presentation, Domain, Data 목표 구조로 전환되었다. 요청받은 기능을 마이그레이션할 때만 목표 구조로 옮긴다. 폴더 정리만을 이유로 요청 범위 밖의 파일을 이동하지 않는다.
