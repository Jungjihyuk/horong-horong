...
- suggestions를 먼저 만든 뒤, 어떤 추천에도 포함되지 않은 항목만 refinements에서 구체화해.

[제외 기준]
- 링크, 경로, 해시태그, 앱 이름, 도구명만 비슷한 항목은 묶지 마.
- 임시 보관, 참고용 메모, 단순 링크 저장처럼 실제 행동 목표가 약한 항목은 묶음에 넣지 마.
- 완료 상태인 할일은 새 목표 후보의 핵심 근거로 삼지 마.
...
- refinements: 다음 추천에 도움이 될 빠진 요소와 한 줄 예시를 써.
...
JSON 형식:
{
  ...
  "refinements": [
    {"items": [3, 4], "missing": ["measurable", "time_bound"], "suggestion": "예: 아침 10시에 30분 러닝"}
  ]
}
