...
- **작은 실험이 시작될 $\color{#D97706}{\textbf{환경}}$ 을 마련하려고** — 좋은 정보를 봐도 *직접 만들어보는* 단계로 넘어가는 장벽은 늘 큽니다. AI Agent CLI를 한 번의 호출로 계획 → 실행까지 이어지게 해, 소비에서 창작으로 가는 다리를 짧게 했습니다. 메모는 목표로, 목표는 비전과 페르소나로 이어지도록 성취 흐름을 시각화해 오늘의 일이 더 큰 방향과 연결되게 했습니다. 데이터는 모두 로컬(SwiftData)에 저장되고 AI 호출도 사용자가 설치한 CLI를 통해 이뤄지기에, 외부 의존 없이 가볍게 시작할 수 있습니다.
...
- 타이머, 메모, 목표, 페르소나/비전, 앱 사용 기록, 통계 캐시, 카테고리 규칙: 로컬 SwiftData 저장소
